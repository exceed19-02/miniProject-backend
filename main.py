...
def update_light(light_object: light):
    ...
    if light_object.mode == "AUTO" or light_object.mode == "DISCO":
        mongo_connection["light"].update_one({"light_id": light_object.light_id},
                                             {"$set": {"status": light_object.status, "mode": light_object.mode}})
        return {"success": True, "detail": "Brightness unchanged"}
    mongo_connection["light"].update_one({"light_id": light_object.light_id}, {
        "$set": {"status": light_object.status, "mode": light_object.mode, "brightness": light_object.brightness}})
    return {"success": True}
